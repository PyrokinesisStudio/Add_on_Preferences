...
class USERPREF_HT_header(Header):
    ...
    def draw(self, context):
        ...
        if userpref.active_section == 'INPUT':
            ...
            # enable / disable all add-ons for debugging purposes
            if bpy.app.debug:
                row = layout.row(align=True)
                row.operator("wm.addon_enable_all", text="Enable All Add-ons").is_enable = True
                row.operator("wm.addon_enable_all", text="Disable All Add-ons").is_enable = False
...
class USERPREF_PT_addons(Panel):
    ...
    def draw(self, context):
        ...
        split = addon_box.split(percentage=0.17)

        col = split.column(align=True)
        row_filter = col.row(align=True)

        col.prop(context.window_manager, "addon_pages_enable", icon='COLLAPSEMENU')
        addon_pages_enable = context.window_manager.addon_pages_enable

        col_pages = col.column(align=True)

        if addon_pages_enable:
            col_pages.prop(context.window_manager, "addon_pages", index=0, text="Page")
            col.prop(context.window_manager, "addon_pages", index=1, text="Page Size")
        ...
        # NOTE: this ugly hack is a consequence of ENUM_FLAG option not working properly with icons
        split_support = col.split(percentage=0.2, align=True)
        sub_col_support = split_support.column(align=True)
        box_support = sub_col_support.box()
        box_support.scale_y = 0.5
        box_support.label(text="", icon='FILE_BLEND')

        box_support = sub_col_support.box()
        box_support.scale_y = 0.5
        box_support.label(text="", icon='POSE_DATA')

        box_support = sub_col_support.box()
        box_support.scale_y = 0.5
        box_support.label(text="", icon='MOD_EXPLODE')

        support_prop = split_support.column(align=True)
        support_prop.prop(context.window_manager, "addon_support")
        # end of ugly hack
        ...
        col_addon = col_split.split(percentage=0.45, align=True)
        col_addons = col_addon.column(align=True)
        box_addons = col_addons.box()

        col = box_addons.column(align=True)
        col.label("Select for info, use the checkbox to enable / disable:")

        col_pref = col_addon.column(align=True)
        box_aprefs = col_pref.box()
        col_prefs = box_aprefs.column(align=True)
        col_prefs.label("Selected Add-on information and preferences:")
        ...
        addon_pages = context.window_manager.addon_pages[0]
        addon_page_size = context.window_manager.addon_pages[1]
        addon_quantity = len(addons)
        addon_current_filter = 0

        # initialized on demand
        user_addon_paths = []

        # is an add-on selected/expanded?
        check_pref_is_expanded = False

        # is an add-on shown with the filtering?
        check_addon_is_shown = False

        # scale the add-on entries height
        button_height = 1.3
        ...
        for mod, info in addons:
            ...
            if ((filter == "All") or
                    ...
                    (filter == "User" and
                    (mod.__file__.startswith((scripts_addons_folder, userpref_addons_folder))))
                    ):
                ...
                addon_current_filter += 1

                if addon_pages_enable:
                    test_page = addon_current_filter / addon_page_size
                    if not (addon_pages - 1 < test_page <= addon_pages):
                        continue

                # if arrived here, an add-on should be displayed
                check_addon_is_shown = True

                # Addon UI Code
                row = col_addons.row(align=True)
                row.scale_y = button_height
                row.alert = info["show_expanded"]
                row.active = is_enabled

                row_toggle = row.row(align=True)
                row_toggle.scale_y = button_height
                row_toggle.alert = info["show_expanded"] if is_enabled else False
                row_toggle.active = is_enabled

                switch_addon = row_toggle.operator(
                        "wm.addon_disable" if is_enabled else "wm.addon_enable",
                        icon='CHECKBOX_HLT' if is_enabled else 'CHECKBOX_DEHLT',
                        text="",
                        emboss=True,
                        )
                switch_addon.module = module_name
                switch_addon.pop_up = True

                row.operator(
                        "wm.addon_expand",
                        text="%s: %s" % (info["category"], info["name"]),
                        emboss=True,
                        icon="ERROR" if info["warning"] else "BLANK1",
                        ).module = module_name
                ...
                row.operator(
                        ...
                        text="",
                        emboss=True,
                        icon=self._support_icon_mapping.get(info["support"], 'QUESTION')
                        ).module = module_name

                # Expanded UI on the right side (only if additional info is available)
                if info["show_expanded"]:
                    check_pref_is_expanded = True
                    box_prefs = col_pref.box()
                    box_prefs.label(
                            text="%s: %s" % (info["category"], info["name"]),
                            icon="INFO"
                            )
                    sub_box_prefs = box_prefs.column(align=True)
                    if info["description"]:
                        split = sub_box_prefs.row().split(percentage=0.15)
                    ...
                    if tot_row:
                        sub_internet = sub_box_prefs.column(align=True)
                        split_internet = sub_internet.split(percentage=0.15)
                        split_internet.label(text="Internet:")
                        split = split_internet.row()
                        if info["wiki_url"]:
        ...
        if not check_pref_is_expanded and check_addon_is_shown:
            col_pref.separator()
            col_pref.label(text="Please select or enable an add-on on the left", icon="INFO")

        row_filter.label("Filtered: %d of %d available" % (addon_current_filter, addon_quantity))

        if addon_current_filter <= addon_page_size:
            context.window_manager.addon_pages[0] = 1
            context.window_manager.addon_pages[2] = 2

        elif context.window_manager.addon_pages[2] == 2:
            context.window_manager.addon_pages[2] = 1

        col_pages.enabled = bool(context.window_manager.addon_pages[2] == 1)

        if not check_addon_is_shown:
            col_split.separator()
            col_split.label(
                    text="No Add-on found matching the filtering / search criteria. "
