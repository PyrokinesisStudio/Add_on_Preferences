...
class WM_OT_addon_enable(Operator):
    ...
    def execute(self, context):
        ...
        if mod:
            ...
            bpy.ops.wm.addon_expand("INVOKE_DEFAULT", module=self.module)

            if self.pop_up is True:
                userpref = bpy.context.user_preferences
                addon_preferences = userpref.addons[self.module].preferences
                if addon_preferences is not None:
                    draw = hasattr(addon_preferences, "draw")
                    if draw is not None:
                        def preferences_notice(self, context):
                            self.layout.label(
                                text="Add-on contains preferences. Please check the Panel on the right",
                                )
                        bpy.context.window_manager.popup_menu(
                                preferences_notice, title="Add-on Preferences", icon='INFO'
                                )
...
class WM_OT_addon_expand(Operator):
    ...
    """Please check the panel on the top right"""
    ...
    def execute(self, context):
        ...
        if mod is not None:
            ...
            if info["show_expanded"] is True:
                """
                context.window_manager.addon_search = info["name"] if \
                        context.window_manager.addon_search != info["name"] else ""
                """
                return {"FINISHED"}

        for mod in addon_utils.addons_fake_modules:
            mods = addon_utils.addons_fake_modules.get(mod)
            if mods is not None:
                info = addon_utils.module_bl_info(mods)
                info["show_expanded"] = True if module_name == mod else False
...
class WM_OT_addon_enable_all(Operator):
    """Toggle the register state of all add-ons"""
    bl_idname = "wm.addon_enable_all"
    bl_label = "Switch the registration state of all Add-ons"
    ...
    exclude_addon = StringProperty(
            name="Exclude Module",
            default="Don't modify this to Enable/Disable all",
            description="Skip register/unregister of this module. Leave as default to apply to all\n"
                        "Use module names (names of the scripts or folders)"
            )
    ...
    def draw(self, context):
        layout = self.layout
        texts = "Enable" if self.is_enable else "Disable"
        layout.label(text="{} Add-on Modules".format(texts), icon="INFO")
        layout.prop(self, "exclude_addon")
    ...
    def execute(self, context):
        ...
        exclusion = self.exclude_addon if self.exclude_addon not in \
                    "Don't modify this to Enable/Disable all" else None
        exclude_broken = ["object_facemap_auto", "development_edit_operator"]

        if exclusion:
            print("\nINFO: Module {} excluded from enable/disable\n".format(exclusion))

        if exclude_broken:
            print("\nINFO: Excepted broken Modules {} excluded from enable/disable\n".format(exclude_broken))

        if self.is_enable:
            print("\n\nAll Add-ons registration start:\n")
            for modules, names in addons:
                try:
                    if exclusion is not None and exclusion in modules.__name__:
                        print("INFO: {} module found. Excluding".format(exclusion))
                        continue
                    elif exclude_broken and modules.__name__ in exclude_broken:
                        print("INFO: {} Excluded broken module found. Excluding".format(modules.__name__))
                        continue
                    print("Registering add-on module {} with the name {}".format(modules.__name__, names["name"]))
                    bpy.ops.wm.addon_enable("EXEC_DEFAULT", module=modules.__name__)
                except Exception as k:
                    print("ERROR REGISTER ALL: module name is: %s" % modules.__name__,
                          ", name is : %s\n" % names["name"], " Issue is %s\n" % k)
                    continue
        else:
            print("\n\nAll Add-ons unregistration start:\n")
            for modules, names in addons:
                try:
                    if exclusion is not None and exclusion in modules.__name__:
                        print("INFO: {} module found. Excluding".format(exclusion))
                        ...
                        print("INFO: {} Excluded broken module found. Excluding".format(modules.__name__))
                        continue
                    print("Unregistering add-on module {} with the name {}".format(modules.__name__, names["name"]))
                    bpy.ops.wm.addon_disable("EXEC_DEFAULT", module=modules.__name__)
                except Exception as e:
                    print("ERROR UNREGISTER ALL: module name is: %s" % modules.__name__,
                          ", name is : %s\n" % names["name"], " Issue is %s\n" % e)
                    continue

        print("\n\nREGISTER / UNREGISTER ALL FINISHED\n\n")
